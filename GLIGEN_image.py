...
# JSON 文件路径
json_file_path = 'blip2-opt-2.7b_image_data.json' 
p = 'prompt_w_suffix'
...
for data_point in blip2_data:
    ...
    if len(data_point["labels"]) == 1 and len(data_point["bboxes"]) <= 6:
        ...
        prompt = f'{len(data_point["bboxes"])} {labels_str} {data_point[p]}' 
        ...
        # 图像文件名
        json_folder_path = check_point_name.split("-")[1] + '_text_image_box' # 这将得到 'opt'
