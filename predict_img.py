...
def plot_result(pil_img, prob, boxes, save_name=None, imshow=False, imwrite=True):
	...
	# 修改成自己要保存的目录
    if imwrite:
        if not os.path.exists("./output/pred07"):
            os.makedirs('./output/pred07')
        cv2.imwrite('./output/pred07/{}'.format(save_name), opencvImage)
...
if __name__ == "__main__":
    ...
    # 加载模型 修改成自己路径
    dfdetr = load_model('checkpoint0049.pth', main_args)  # <--修改为自己加载模型的路径
    # <--修改为待预测图片所在文件夹路径
